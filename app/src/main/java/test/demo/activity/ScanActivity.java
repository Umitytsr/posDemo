        ...
        mScanRecevier = new BroadcastReceiver() {
            public void onReceive(Context context, Intent intent) {
                ...
                if(encodeType == ENCODE_MODE_NONE){
                    //byte[] bresult = intent.getByteArrayExtra("EXTRA_SCAN_DATA");
                    byte[] bresult = intent.getByteArrayExtra("EXTRA_SCAN_RAW_DATA");
                    length = intent.getIntExtra("EXTRA_SCAN_RAW_DATA_LEN",0);
                      /*for(int i = 0 ; i < resultLen; i++){
                         scanResult += "" + bresult[i];
                      }*/
                    scanResult = ByteUtil.bytearrayToHexString(bresult, length);
                }else {

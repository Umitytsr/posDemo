...
public class FiscalActivity extends Activity implements View.OnClickListener{
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                case MSG_FISCAL_OPEN:
                    int ret;
                    try {
                        ret = MyApplication.app.taxOpt.fiscalOpen(115200,8,1,'N','N');
                        if(ret == 0){
                ...
                case MSG_FISCAL_CLOSE:
                    int close;
                    try {
                        close = MyApplication.app.taxOpt.fiscalClose();
                        if(close == 0){
                            runOnUiThread(new Runnable() {
                                public void run() {
    ...
    private void fiscaltest(){
        ...
        try {
            ret = MyApplication.app.taxOpt.fiscalWrite(cmd);
            if(ret == 0){
                byte[] buffer = new byte[36];
                //ret = posApiHelper.fiscalRead(buffer,36,500);
                ret = MyApplication.app.taxOpt.fiscalRead(buffer,26,500);
